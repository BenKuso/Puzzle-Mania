...
void printmapa(BITMAP *bobbles[], BITMAP *buffer){  
	...
	for(int i=0; i<10; i++){
		for(int j=0; j<8; j++){
			...
			if(color!=0){
				if(i%2==0){
					draw_sprite(buffer, bobbles[color], j*50+297, i*45+73);
				}else{
					draw_sprite(buffer, bobbles[color], j*50+327, i*45+73);
				}
			}	
		}
	}
...
int colision(int px, int py, int c){
	...
	j=py/60;
//	system("pause");
	if(mapa[i-1][j]=='v'||mapa[i-1][j]=='z'||mapa[i-1][j]=='a'||mapa[i-1][j]=='r'||mapa[i-1][j]=='g'||mapa[i-1][j]=='b'){
		switch(c){
			case 1:
				mapa[i][j]=='r';
				break;
